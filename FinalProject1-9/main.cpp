...
// Глобальные переменные отвечающие за работу, авторизацию и пользователя.
bool work = true;
...
int main() {
	...
	char userChoice_work;

	while (work) { // ГЛАВНОЕ МЕНЮ
		...
		case '3':
			...
			cout << "Подробная информация о программе находится в Readme." << endl;
			break;
...
void singleChat() { // ПРИВАТНЫЙ ЧАТ
	...
	if (to != "") {
		...
		while (message != "0") {
			...
			else if (message != "0") { // Отправляем сообщение
